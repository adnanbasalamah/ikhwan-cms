...
//4. delete Video
	...
	{	
		...
		foreach ($videos as $video)
		{
			$this->table->add_row(
				++$i, 
				'<img src="'.$video->screenshoot_url.'" "width="160" height="90" alt="">', 
				$video->video_title, 
				$video->video_teaser,
				...
				anchor('video/update/'.$video->id,'<button class="btn btn-primary" type="submit">Edit</button>',array('class'=>'update')).' '.
				anchor('video/delete/'.$video->id,'<button class="btn btn-danger" type="submit">Delete</button></td>',array('class'=>'delete','onclick'=>"return confirm('Are you sure want to delete this video?')"))
			);
		}
	...
	function add()
	{
		...
		$data['message'] = '';
		$data['action'] = site_url('video/add_video/');
	...
	{   // form handler utk add video
		...
		{	
			...
			$video = array(	'video_title' 		=> $this->input->post('title'),
							...
							'video_url' 		=> prep_url($this->input->post('videourl')),
	...
	function update($id)
	{
		...
		$data['screenshoot_url'] 	= $video->screenshoot_url;
